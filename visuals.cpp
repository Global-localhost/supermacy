...
void Visuals::StatusIndicators() {
	...
	// OVERRIDE DMG
	if (g_menu.main.visuals.indicators.get(3) && g_input.GetKeyState(g_menu.main.aimbot.override_dmg_key.get())) {
		Indicator_t ind{ };
	...
	for (size_t i{ }; i < indicators.size(); ++i) {
		...
		render::indicator.string(10, g_cl.m_height - 80 - (30 * i), indicator.color, indicator.text);
	}
}
	...
	// draw flags.
	{
		...
		for (auto it = items.begin(); it != items.end(); ++it) {
			...
			if (*it == 6 && IsFakeDucking(player)) {
				flags.push_back({ XOR("FAKEDUCK"), { 255, 0, 0, low_alpha } });
			}
		}
