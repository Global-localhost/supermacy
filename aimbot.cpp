...
void Aimbot::find() {
	...
	if (best.player && best.record) {
		...
		// if we can scope.
		bool can_scope = !g_cl.m_local->m_bIsScoped() && (g_cl.m_weapon_id == AUG || g_cl.m_weapon_id == SG553 || g_cl.m_weapon_id == G3SG1 || g_cl.m_weapon_id == AWP || g_cl.m_weapon_id == SSG08 || g_cl.m_weapon_id == SCAR20);
...
bool AimPlayer::GetBestAimPosition(vec3_t& aim, float& damage, LagRecord* record, int& hitbox) {
	...
	if (g_cl.m_weapon_id == ZEUS) {
		...
		if (g_input.GetKeyState(g_menu.main.aimbot.override_dmg_key.get())) {
